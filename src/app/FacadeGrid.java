...
public class FacadeGrid {
	...
	public static void execute() {
		...
		while (cond) {
			...
			cond = FacadeGrid.menu(opc);
		}
	}
	
	/*Método que retona o menu com sua estrutura de decisão*/
	public static boolean menu(String opc){
		boolean cond = true;
		...
		try {
			if (opc.equals("4")) {
				...
				if (input_key.length() == 9) {
					...
					JOptionPane.showMessageDialog(null,
							"Ops! Estamos trabalhando com uma grid 3x3, logo precisamos de 9 caracteres.\n Por favor, insira exatamente 9 caracteres.");
				}
				...
				if (input_key.length() == 16) {
					...
					JOptionPane.showMessageDialog(null,
							"Ops! Estamos trabalhando com uma grid 4x4, logo precisamos de 16 caracteres.\n Por favor, insira exatamente 16 caracteres.");
				}
			}
